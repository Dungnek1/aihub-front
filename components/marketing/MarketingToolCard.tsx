"use client";
import { Star, MoreHorizontal, Megaphone, Bookmark, ArrowRight } from "lucide-react";
...
import { useSavedMarketingTools } from "@/contexts/SavedMarketingToolsContext";

type Props = { tool: Tool };

export default function MarketingToolCard({ tool }: Props) {
  const t = useTranslations("AITools");
  const toast = useToast();
  const router = useRouter();
  const { user, isAuthenticated } = useAuth();
  const { isMarketingToolSaved, toggleSave, refreshSavedMarketingTools } = useSavedMarketingTools();
  ...
  const isSaved = isMarketingToolSaved(tool.id);

  // Handle save/unsave marketing tool
  const handleToggleSave = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // 1. Tool was unsaved (isNowSaved = false)
      ...
      if (isNowSaved !== undefined) {
        ...
        // Refresh saved marketing tools context to update UI
        await refreshSavedMarketingTools();
        router.refresh();
      } else {
      ...
      if (
        ...
        toast.error(t("saveError") || "Lỗi khi lưu công cụ");
        console.error("Save marketing tool error:", error);
      }
    } finally {
  ...
  return (
    <>
      ...
      >
        <article
          className="rounded-2xl overflow-hidden border border-white/10 bg-[#0A0F18] hover:border-cyan-400/40 transition-all cursor-pointer h-full flex flex-col shadow-[0_10px_30px_rgba(0,0,0,0.35)] hover:shadow-[0_18px_35px_rgba(23,239,247,0.25)]"
        ...
        >
            ...
            {!tool.logoUrl || imageError ? (
              ...
              <img
                ...
                onError={async (e) => {
                  ...
                  // Image load error - silently handle by showing placeholder
                  // Only log in development if needed for debugging
                  if (process.env.NODE_ENV === "development" && process.env.NEXT_PUBLIC_DEBUG_IMAGES === "true") {
                    console.error("[MarketingToolCard] Image load error:", {
                      src: normalizedSrc,
                      toolId: tool.id,
                      toolName: tool.name,
                    });
          ...
          <div className="p-3 sm:p-4 flex-1 flex flex-col">
            ...
            <div className="mt-1 flex items-center gap-3 text-sm flex-wrap">
              ...
              <span className="text-white/90 font-semibold min-w-[32px]">
                {avgRating.toFixed(1)}
              </span>
              {tool.price && (
                <span
            ...
            {tool.description && (
              <p className="mt-2 text-xs text-white/70 line-clamp-2 flex-1">
                {tool.description}
              </p>
            )}
            {/* Try Now Button */}
