...
import { rateCourse } from "@/services/client/courses.client";
...
export default function CourseRatingModal({
  ...
  const [learningOutcome, setLearningOutcome] = useState<number>(5);
  const [materialsQuality, setMaterialsQuality] = useState<number>(5);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setLearningOutcome(5);
      setMaterialsQuality(5);
    }
  }, [open]);
  ...
  const handleRate = async () => {
    ...
    try {
      const response = await rateCourse(course.id, {
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 id="rating-modal-title" className="text-xl font-semibold">{course.name}</h2>
